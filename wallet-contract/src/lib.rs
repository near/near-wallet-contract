...
use near_sdk::{
    AccountId, Allowance, Gas, GasWeight, NearToken, Promise, PromiseError, PromiseOrValue, env,
    json_types::U64, near,
};
...
/// Maximum size of the result value from a cross-contract call that we will read.
/// This prevents OOG attacks where a malicious contract returns a huge payload
/// to cause the callback to run out of gas.
///
/// The limit is derived from RLP_EXECUTE_CALLBACK_GAS (5 Tgas) and the cost of reading
/// from a register. Reading a register costs ~5.7 Ggas base + ~5.6 Mgas per byte.
/// For 16KB: 5.7 Ggas + (16 * 1024 * 5.6 Mgas) ≈ 5.7 Ggas + 92 Ggas ≈ 98 Ggas.
/// With 5 Tgas available, we could theoretically read ~900KB, but we use a conservative
/// 16KB limit to leave ample headroom for other callback operations (JSON parsing,
...
impl WalletContract {
    ...
    pub fn rlp_execute(
        ...
        if self.has_in_flight_tx {
            return PromiseOrValue::Value(ExecuteResponse {
                ...
                success_value: None,
                error: Some(
                    "Error: transaction already in progress, please try again later.".to_string(),
                ),
            });
        }
    ...
    pub fn address_check_callback(
        ...
        // Use promise_result_checked to prevent OOG attacks from large return values.
        // The registrar response is just an Option<AccountId>. AccountId max is 64 chars,
        ...
        let maybe_account_id: Option<AccountId> = match env::promise_result_checked(0, 256) {
            Err(PromiseError::Failed) => {
                return PromiseOrValue::Value(ExecuteResponse {
                    success: false,
                    success_value: None,
                    error: Some("Call to Address Registrar contract failed".to_string()),
                });
            }
            Err(PromiseError::TooLong(_)) | Err(_) => {
                return PromiseOrValue::Value(ExecuteResponse {
                    success: false,
                    success_value: None,
                    error: Some("Unexpected response size from account registrar".to_string()),
                });
            }
            Ok(value) => match serde_json::from_slice(&value) {
                ...
                Err(_) => {
                    return PromiseOrValue::Value(ExecuteResponse {
                        success: false,
                        success_value: None,
                        error: Some("Unexpected response from account registrar".to_string()),
                    });
                }
            },
        };
        ...
        let promise = if maybe_account_id.is_some() {
            // Faulty relayer: address is registered but relayer used eth-implicit target.
            if env::signer_account_id() == current_account_id {
                create_ban_relayer_promise(current_account_id)
            } else {
                // External caller: increment nonce to prevent replay attacks.
                // Even though this is a relayer error, we must increment the nonce here
                // because the refund was already paid in `inner_rlp_execute`. Without
                ...
                return PromiseOrValue::Value(ExecuteResponse {
                    success: false,
                    success_value: None,
                    error: Some("Invalid target: target is address corresponding to existing named account_id".to_string()),
                });
            }
        } else {
    ...
    pub fn nep_141_storage_balance_callback(
        ...
        self.has_in_flight_tx = false;
        // Use promise_result_checked to prevent OOG attacks from large return values.
        // StorageBalance contains two U128 values (total and available). u128::MAX is 39 digits,
        // plus JSON formatting overhead, so 256 bytes is plenty.
            ...
            match env::promise_result_checked(0, 256) {
                Err(PromiseError::Failed) => {
                    return PromiseOrValue::Value(ExecuteResponse {
                        ...
                        success_value: None,
                        error: Some(format!(
                            "Call to NEP-141 {token_id}::storage_balance_of failed"
                        )),
                    });
                }
                Err(PromiseError::TooLong(_)) | Err(_) => {
                    return PromiseOrValue::Value(ExecuteResponse {
                        ...
                        success_value: None,
                        error: Some(
                            "Unexpected response size from NEP-141 storage_balance_of".to_string(),
                        ),
                    });
                }
                Ok(value) => match serde_json::from_slice(&value) {
                    ...
                    Err(_) => {
                        return PromiseOrValue::Value(ExecuteResponse {
                            ...
                            success_value: None,
                            error: Some(
                                "Unexpected response from NEP-141 storage_balance_of".to_string(),
                            ),
                        });
                    }
                },
                ...
                let transfer_function_call = match action {
                    ...
                    _ => {
                        return PromiseOrValue::Value(ExecuteResponse {
                            ...
                            success_value: None,
                            error: Some(
                                "Expected function call action to perform NEP-141 transfer"
                                    .to_string(),
                            ),
                        });
                    }
    ...
    pub fn rlp_execute_callback(
        ...
        // Use promise_result_checked to bound the size of returned data.
        // This prevents OOG attacks where a malicious contract returns a huge payload.
        match env::promise_result_checked(0, MAX_PROMISE_RESULT_SIZE) {
            Ok(value) => ExecuteResponse {
                success: true,
                success_value: Some(value),
                error: None,
            },
            Err(PromiseError::Failed) => {
                ...
                ExecuteResponse {
                    success: false,
                    success_value: None,
                    error: Some("Failed Near promise".to_string()),
                }
            }
            Err(PromiseError::TooLong(len)) => {
                // Result was too large - treat as success but truncate the value.
                // NOTE: This creates a case where success=true but success_value=None,
                // which breaks the previous invariant that success implies a value.
                // Downstream consumers (e.g., Aurora's web3 relayer) should handle this
                // case where the promise itself succeeded but the result was too large
                // to read safely.
                ExecuteResponse {
                    success: true,
                    success_value: None,
                    error: Some(format!(
                        "Result truncated: response size {} exceeds limit {}",
                        len, MAX_PROMISE_RESULT_SIZE
                    )),
                }
            }
            // Handle any future PromiseError variants (non_exhaustive).
            // Be conservative and treat unknown errors as failures.
            Err(_) => ExecuteResponse {
                success: false,
                success_value: None,
                error: Some("Unknown promise error".to_string()),
            },
        }
    }
    ...
    pub fn ban_relayer(&mut self) -> ExecuteResponse {
        ...
        ExecuteResponse {
            success: false,
            success_value: None,
            error: Some("Error: faulty relayer".to_string()),
        }
    }
}
...
fn inner_rlp_execute(
    ...
    let promise = match transaction_kind {
        TransactionKind::EthEmulation(EthEmulationKind::EOABaseTokenTransfer {
            ...
            address_registrar
                .lookup(address)
                .then(ext.address_check_callback(target, action, caller_deposit))
        }
