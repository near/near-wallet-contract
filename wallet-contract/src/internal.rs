use crate::{
    ...
    types::{
        ...
        ParsableTransactionKind, TRANSFER_SELECTOR, TRANSFER_SIGNATURE, TargetKind,
        TransactionKind,
    },
};
...
fn parse_tx_data(
    ...
    match &tx.data[0..4] {
        ...
        _ => {
            ...
            Ok((
                action,
                ParsableTransactionKind::EthEmulation(emulation_kind),
            ))
        }
    }
}
...
fn validate_tx_relayer_data<'a>(
        ...
        TargetKind::EthImplicit(address) if to == address => {
            target.as_str()
                == format!(
                    "0x{}{}",
                    hex::encode(address),
                    context.current_account_suffix()
                )
        }
    ...
    let gas_limit = if tx.gas_limit < U64_MAX {
        tx.gas_limit.as_u64()
    } else {
        u64::MAX
    };
